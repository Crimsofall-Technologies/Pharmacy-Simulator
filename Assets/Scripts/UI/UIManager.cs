...
public class UIManager : MonoBehaviour
{
	...
	public Animator researchUIAnimator, taskAnimator, achieveAnimator, achieveCompAnimator;
	...
	public GameObject groceryButton, iceCreamButton, pharmacyButton, drinksButton, backRoomsButton, vaccineButton, emptyUI, shopUI, backRoomGO, levelUpGO, buildGO, researchUI, achieveCompleteUI;
	public Text groceryText, iceCreamText, pharmacyText, drinksText, levelText, backRoomsText, vaccineText, achieveCompText;
    ...
    private void Update()
    {
		...
		if (Input.GetKeyDown(KeyCode.V)) { GlobalVar.Instance.AddGems(100); }
    }
    ...
    public void UpdateUI()
	{
        ...
        pharmacyButton.gameObject.SetActive(shop.pharmacyAmount <= 0 && !shop.refillingPharmacy);
		drinksButton.gameObject.SetActive(shop.drinksAmount <= 0 && !shop.refillingDrinks);
		groceryButton.gameObject.SetActive(shop.groceriesAmount <= 0 && !shop.refillingGroceries);
		iceCreamButton.gameObject.SetActive(shop.iceCreamAmount <= 0 && !shop.refillingIceCream);
        vaccineButton.gameObject.SetActive(shop.vaccineAmount <= 0 && !shop.refillingVaccines);

        if (TutorialManager.Instance.TutorialRunning && (pharmacyButton.activeSelf || drinksButton.gameObject.activeSelf || groceryButton.gameObject.activeSelf || iceCreamButton.gameObject.activeSelf))
		{
	...
	private IEnumerator UpdateBarsSmoothly()
	{
		while(true)
		{
			float value = GlobalVar.Instance.currentXP / GlobalVar.Instance.nextXP;
			yield return new WaitForSeconds(0.005f);
			if(taskFill.value != value)
			{
				if(taskFill.value < value)
				...
				if(taskFill.value > value) taskFill.value = value;
	...
	public void CreateTimerUI(Timer timer, Vector3 pos, int time, Sprite spr)
	{
		TimerUI ui = Instantiate(timerUIPrefab, timerUIArea).GetComponent<TimerUI>();
		if (spr != null) ui.Icon.sprite = spr;
		ui.Init(pos, time);
    ...
    public void OpenAchieveUI(string achieveName)
    {
		...
		achieveCompText.text = "You hae completed:\n"+achieveName+"!";
    }
