...
public class GameManager : MonoBehaviour
{
	...
	public void LevelUp()
	{
		...
		GlobalVar.Instance.nextXP = (int)(8.55f * Level);
	...
	public void Refill(int index)
	{
		...
		int time = DebugMode ? 5 : 600;
		
		if(index == 0) { 
			timerName = "G_Timer"; 
			shop.refillingGroceries = true;
			ui.groceryText.text = GetSortedTimeFromSeconds(time);
		}
		if(index == 1) { 
			timerName = "P_Timer"; 
			shop.refillingPharmacy = true;
			ui.pharmacyText.text = GetSortedTimeFromSeconds(time);
		}
		if(index == 2) { 
			timerName = "I_Timer"; 
			shop.refillingIceCream = true;
			ui.iceCreamText.text = GetSortedTimeFromSeconds(time);
		}
		if(index == 3) { 
			timerName = "D_Timer"; 
			shop.refillingDrinks = true;
			ui.drinksText.text = GetSortedTimeFromSeconds(time);
		}
        if (index == 3)
        {
            timerName = "V_Timer";
            shop.refillingVaccines = true;
            ui.vaccineText.text = GetSortedTimeFromSeconds(time);
        }

        //fill delayed based on timing.
        Timer timer = TimerManager.CreateTimer(timerName, time, new Timer.CustomData(){ _int = index }, (GameManager.Instance.perksManager.DoubleSpeed ? 2f : 1f));
	...
	private void OnRefillTimerMoved(Timer.CustomData data, int remainingTime)
	{
		...
		if (data._int == 0)
			ui.groceryText.text = GetSortedTimeFromSeconds(remainingTime);
