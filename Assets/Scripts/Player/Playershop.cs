...
public class Playershop : MonoBehaviour
{
	...
	public class ShopPosition
	{
		...
		private bool[] areasRemaining;
		
		public void Init()
		{
			areasRemaining = new bool[positions.Length];
		}
		
		public bool HasEmptyArea()
		{
			for (int i = 0; i < areasRemaining.Length; i++) {
				if(!areasRemaining[i]) return true;
			}
			return false;
		}
		...
		public Transform GetNPCPosition()
		{
			for (int i = 0; i < areasRemaining.Length; i++) {
				...
				if(areasRemaining[m] == false && positions[m].GetComponent<TargetPointer>().CanStandHere())
				{
					return positions[m];
				}
			}
			
			//fallback - return closest open one:
			for (int i = 0; i < areasRemaining.Length; i++) {
				if(areasRemaining[i] == false && positions[i].GetComponent<TargetPointer>().CanStandHere())
				{
					return positions[i];
				}
			}
			
			//completely fallback (this is really not possible in any way)
			return null;
		}
		
		public void OnAreaOccupationChanged(Transform T, bool value)
		{
			for (int i = 0; i < positions.Length; i++) {
				if(positions[i] == T)
				{
					areasRemaining[i] = value;
				}
			}
		}
		
		public bool IsAreaOccupied(Transform T)
		{
			for (int i = 0; i < positions.Length; i++) {
				if(positions[i] == T)
				{
			...
			//fallback:
			return true; //just return the area is occupied.
		}
	}
	...
	public void OnShopperOrderCompleted(Shopper shopper, Transform T)
	{
		...
		for (int i = 0; i < shopPositions.Length; i++) {
			...
			{
				shopPositions[i].OnAreaOccupationChanged(T, false);
				return;
			}
		}
	}
	...
	public Transform GetClosePosition(ShopperType Type)
	{
		for (int i = 0; i < shopPositions.Length; i++) {
	...
	public ShopperType[] GetShopperTypes(bool isThief)
	{
		if (isThief) 
		{
            ...
            if (Random.value <= 0.5f && creatableBuilding.IsActive(ShopperType.IceCream) && shopPositions[1].GetNPCPosition() != null) 
	...
	public void AreaOccupied(Shopper shopper, Transform T)
	{
		for (int i = 0; i < shopPositions.Length; i++) {
			if(shopPositions[i].shopperType == shopper.Types[shopper.currentIndex])
			{
				shopPositions[i].OnAreaOccupationChanged(T, true);
				return;
			}
		}
	}
	...
	public bool IsAreaOccupied(Transform T, ShopperType shopperType)
	{
		for (int i = 0; i < shopPositions.Length; i++) {
			if(shopPositions[i].shopperType == shopperType && shopPositions[i].HasEmptyArea())
			{
				return shopPositions[i].IsAreaOccupied(T);
			}
		}
